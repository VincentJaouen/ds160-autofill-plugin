...
$( document ).ready(function() {
    chrome.runtime.onMessage.addListener(function(request, sender) {
  		if(request.from=="background" && request.data){
  			...
  			if(personalData){
  				// chrome.storage.sync.set({'userData': personalData}, function() {
					syncStore('userData', personalData, function() {
					...
					chrome.storage.sync.get("startStatus", function (obj) {
						if(obj.startStatus=="pause"){
						}else if(obj.startStatus=="start"){
							setTimeout(function(){
		...
		if(location.href.indexOf("complete_personalcont.aspx?node=Personal2")!=-1){
			...
			getStore("userData", function (obj) {
			// chrome.storage.sync.get("userData", function (obj) {
				var personalData = obj;
