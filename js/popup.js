jQuery(document).ready(function () {
	...
	$("#btn_stop").click(function(){
		chrome.storage.sync.set({'startStatus': 'pause'}, function() {
		});
		$("#btn_stop").hide();
		$("#btn_restart").show();
	});
	$("#btn_restart").click(function(){
		...
		chrome.tabs.query({active: true, currentWindow: true}, function(tabs) {
		    ...
		    chrome.tabs.getSelected(null, function(tab) {
		        chrome.extension.sendMessage({from: "restart", tabId: tab.id},function(response) {
				});
		    });
		});
	...
	//26266
	var api_url = "https://www.oliver.ai/process_users/";
	$("#btn_submit").click(function(){
		if(!$("#user_id").val()){
			...
			chrome.extension.sendMessage({from: "popup", tabId: tabId, url:api_url + user_id+'.json'},function(response) {
			});
		}
	});
});
